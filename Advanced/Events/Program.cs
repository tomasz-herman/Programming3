...
class Program
{
    ...
    private static void StockOnPriceChanged(string symbol, decimal oldprice, decimal newprice)
    {
        decimal change = (newprice - oldprice) / oldprice;
        Console.ForegroundColor = change < 0 ? ConsoleColor.Red : ConsoleColor.Green;
        Console.WriteLine($"{symbol,4}: {change:P2}");
        Console.ResetColor();
    }
